...
if __name__=="__main__":
    ...
    baseband_log_dir=config_file.get("albatros2", "baseband log directory")
    if not os.path.isdir(baseband_log_dir):
        os.mkdir(baseband_log_dir)
    baseband_log_name=config_file.get("albatros2", "baseband log name")
    ...
    drive_safety=int(config_file.get("albatros2", "drive safety"))
    logger.info("# (6) Drive safety percentage: %d"%(drive_safety))
    file_size=float(config_file.get("albatros2", "file size"))
    logger.info("# (7) File size: %f"%(file_size))
    baseband_directory_name=config_file.get("albatros2", "baseband directory name")
