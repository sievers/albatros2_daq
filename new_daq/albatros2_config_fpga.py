...
if __name__=="__main__":
    ...
    log_dir=config_file.get("albatros2", "init log directory")
    if not os.path.isdir(log_dir):
        os.mkdir(log_dir)
    log_name=config_file.get("albatros2", "init log name")
    ...
    channels_coeffs=config_file.get("albatros2", "channels coeffs")
    logger.info("# (13) Four bit channel coeffs: %s"%(channels_coeffs))
    logger.info("# (14) Start of log file name: %s"%(log_name+"_%d%m%Y%_%H%M%S"))
    logger.info("# (15) Log directory: %s"%(log_dir))
    logger.info("########################################################################################")
    drives_full=config_file.get("albatros2", "drives full")
    if drives_full=="true":
